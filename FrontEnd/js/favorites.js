    ...
    <div class="product-card-body">
      ...
      <div class="product-card-footer">
        ...
        <button class="product-add-btn" aria-label="Adicionar ao carrinho"
          onclick="event.preventDefault(); addToCartFromFav(${JSON.stringify(p).replace(/"/g, '&quot;')}, '${p.id}')">
...
async function renderFavorites() {
  let favs = VaultStore.getFavorites();
  if (isLoggedIn() && window.FavoritosUI) {
    try {
      favs = await window.FavoritosUI.getFavorites();
    } catch (err) {
...
async function removeFav(productId) {
  if (isLoggedIn() && window.FavoritosUI) {
    await window.FavoritosUI.remover(productId);
...
async function addToCartFromFav(product, itemId) {
  if (typeof window.vaultAddToCart === 'function') {
    await window.vaultAddToCart(itemId || product.id, product);
    return;
  }
...
async function clearAllFavorites() {
  ...
  if (isLoggedIn() && window.FavoritosUI) {
    const favs = await window.FavoritosUI.getFavorites();
    for (const p of favs) {
      await window.FavoritosUI.remover(p.id);
    }
    await renderFavorites();
    return;
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('favoritos:ready', () => renderFavorites());
  document.addEventListener('favoritos:updated', () => renderFavorites());
});
