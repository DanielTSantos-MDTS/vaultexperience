...
async function addToCart(qty = 1) {
  if (localStorage.getItem('vault_token') && window.CarrinhoUI) {
    await window.CarrinhoUI.adicionarAoCarrinho(PRODUTO.id, qty);
    return;
  }
...
async function toggleFav(btnEl) {
  const product = {
    id:    PRODUTO.id,
    name:  PRODUTO.titulo,
    price: PRODUTO.precoAtual,
  ...
  if (localStorage.getItem('vault_token') && window.FavoritosUI) {
    await window.FavoritosUI.toggleFavorito(btnEl, PRODUTO.id);
    return;
  }
...
function renderLanceSection(p) {
  if (!p.negociacao || !p.id) return '';
  ...
  return `
      <div class="lance-section">
        ...
        <p class="lance-floor-hint">Lance mínimo: <strong data-price-floor>R$ ${fmtPreco(p.precoAtual)}</strong></p>
        <form data-form-lance data-item-id="${p.id}" class="lance-form">
          <div class="lance-chips">
            <button type="button" class="lance-chip" data-add="50">+ R$ 50,00</button>
            <button type="button" class="lance-chip" data-add="100">+ R$ 100,00</button>
            <button type="button" class="lance-chip" data-add="150">+ R$ 150,00</button>
            <button type="button" class="lance-chip" data-add="200">+ R$ 200,00</button>
          </div>
          <div class="lance-input-row">
            <span class="lance-prefix">R$</span>
            <input id="lance-input" name="valor" class="lance-input" type="number"
              placeholder="Digite seu valor" min="${p.precoAtual}" step="0.01">
            <button type="submit" id="btn-lance" class="btn-lance" disabled>
              <svg width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path d="M14.5 17.5L3 6V3h3l11.5 11.5"/><path d="M13 19l6-6"/><path d="M2 21l4.5-4.5"/>
              </svg>
              Dar lance
        ...
        <div data-lances-container data-item-id="${p.id}"></div>
      </div>`;
}
  ...
  return `
    <div class="purchase-box">
      ...
      <div class="cta-stack">
        ...
        <button class="btn-wishlist ${isFaved() ? 'faved' : ''}" id="btn-wishlist" data-item-id="${PRODUTO.id}">
...
function renderPage() {
  ...
  bindProdutoEvents();
  bindComments();
  document.dispatchEvent(new CustomEvent('produto:rendered', { detail: { itemId: PRODUTO.id } }));
}
function bindProdutoEvents() {
  const p = PRODUTO;
  ...
  document.querySelectorAll('.lance-chip').forEach(chip => {
    chip.addEventListener('click', () => {
      ...
      const min     = parseFloat(lanceInput.min) || 0;
      const current = parseFloat(lanceInput.value) || min;
  ...
  lanceInput?.addEventListener('input', () => {
    ...
    const min = parseFloat(lanceInput.min) || 0;
    if (btnLance) btnLance.disabled = !(val > min);
  });
}
...
function preencherProduto(dados) {
  ...
  PRODUTO.negociacao = Boolean(dados.negociacao);
}
