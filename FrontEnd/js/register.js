...
registerForm.addEventListener('submit', (e) => {
  ...
  .then(dados => {
    if(dados.Erro) {
      ...
      alert("Registro Concluído com sucesso! Você será redirecionado.");
      saveLocalStorage(dados);
      // redirect handled inside saveLocalStorage
    }
  })
  .catch(erro => {
...
function saveLocalStorage(dados){
  if (dados.token) localStorage.setItem('vault_token', dados.token);
  localStorage.setItem('vault_user', dados.username);
  if (dados.dataNascimento) localStorage.setItem('vault_nascimento', dados.dataNascimento);
  if (dados.contatoPrincipal) localStorage.setItem('vault_contato', dados.contatoPrincipal);
